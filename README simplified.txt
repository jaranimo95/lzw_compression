...
####################################
 ...
 [X] 2. Make MyLZW
 [ ] 3. Read/understand provided code
 [ ] 4. Changes to LZW in MyLZW
		
		- Algorithm will vary size of out -> in codewords from 9 -> 16 bits
		
		- Codeword size should increase when all previous ones are used up
		...
		- Three Options upon Filled Codebook:
			...
			a. Do Nothing: continue using full codebook (already done in LZW.java)

			b. Reset: rest dict back to initial state for new codewords. 
				...
				- One idea is the have an extra output file for each of the compress() and expand() methods to output any 	 trace code. Printing out (codeword, string) pairs in the iterations just before and after a bit change  	or reset is done can help you a lot to synchronize your code properly.
			
			c. Monitor: Begin monitoring the compression ration whenever filling the codebook
				- Begin recording compression ratios whenever the codebook is filled and do nothing until then
				- Compression ratio defined as the size of the uncompressed data that has been read in so far divided by
					the size of the compressed data processed so far
				- If the ratio degrades beyond a certain threshold from when the last codeword was added and reset the dict
					back to its initial state (much like in reset mode). The threshold is defined as a ratio of compression
					ratios
			...
			% The mode will be chosen by the program and recorded at the beginning of the output file pre-compression so 	that it can be automatically retrieved upon expansion. Compression/expansion should use the same mode.
			...
			% LZW already provides command line processing, and i/o files provide their own i/o redirection (&lt for input 		file, &gt for output file), so new arguments n,r,m should be handled in much the same way:
